...
endereco_dados = 'DADOS\Financeira.csv'
...
if len(df_financeira_Vlr_emprestado_outliers_superiores) == 0:
    ...
    plt.subplost(2,2,figsize=(16,7))
    plt.suptitle('Analise os Dados sobre Renda x Emprestimo')

    plt.subplot(2,2,1)
    plt;title('lista das Rendas por Clientes')
    plt.bar(df_financeira['id_cliente'],df_financeira['Renda'])

#posiçao 01: grafico dos emprestimos
    plt.subplot(2,2,1)
    plt;title('lista dos emprestimos')
    plt.bar(df_financeira['id_cliente'],df_financeira['Vlr_emprestimo'])

    #posicao 02: medidas descretivas das rendas
